...
def _generate_hypercube(samples, dimensions, rng):
    ...
    ----------
    samples : int
        Number of distinct binary samples to generate.
    dimensions : int
        Length of each binary sample (number of features).
    ...
    -------
    out: numpy.ndarray
        Array of shape (samples, dimensions) containing binary values (0 or 1).
        Each row represents a unique binary sample.
    """
...
def _generate_leaky_variables(
    ...
    sensitive_data : np.array
        Array of integer values representing categories (e.g., [0, 1, 2, 1, 0, 2]).

    sensitive_probabilities : dict
        Dictionary mapping Z values to their prediction accuracy (0-1).
        Higher values mean the leaky variables will be more predictive of that group.
        Example: {0: 0.8, 1: 0.6, 2: 0.9} means category 0 has 80% accuracy rate, etc.
    
    n_leaky : int
        Number of independent X variables to generate.
    
    return_weights : bool, default=False
        If True, the function will return the weights used in the formula.

    
    formula : bool, default=False
        If True, the function will print the formula used to generate the data.
    
    Returns:
    -------
    X_l : np.array of shape (n_samples, n_leaky)
        Generated leaky variables matrix. Each column represents one leaky feature,
        and each row corresponds to a sample from the input sensitive_data.

    weights : dict (optional)
        Dictionary containing the weights used in the formula.
    """
...
def get_group_name(unique_groups, group):
    ...
    unique_groups : ndarray
        Array containing all unique group identifiers in the dataset.
        The length determines which naming convention to use.
        Example: [0, 1] or [0, 1, 2, 3]
    
    group : int
        Numerical identifier representing a specific group.
...
def get_class_name(cls):
    ...
    ----------
    cls : int
       Numerical class identifier representing a class (0 or 1).
...
def get_params_for_certain_equality_type(equality_type, sensitive_group_count):
    ...
    sensitive_group_count : int
        Number of sensitive groups in the dataset (2, 3, 4, or 5).
        Determines which groups will be included and their parameters.
    
    Returns:
    -------
    dict
        Dictionary containing parameters for each group.
        Parameters may include:
        
        - 'class_sep' : float
            Controls class separability (affects classification difficulty).
            Higher values = easier classification for that group.
            
        - 'weights' : list of float
            Controls class distribution [negative_weight, positive_weight].
            Affects the proportion of positive vs negative samples.
...
def get_group_marker(group):
    ...
    -------
    str
        Matplotlib marker string. One of: 'o', 's', '^', 'D', '*', 'v', 
        '<', '>', 'p', 'h', 'H', '+', 'x', '8'
    """
